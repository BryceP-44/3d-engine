...
info=[[-1,0,0],[-1,1,0],[-1,0,1],[-1,1,1],[1,0,0],[1,1,0],[1,0,1],[1,1,1]]
...
while cont==1:
    for i in range(len(info)):
        ...
        if phiz-betaz>=theta and phiz-betaz<=180-theta:
            xp=(rxz*cos(phiz-betaz)-obsx)/(2*dyz+dd)*tan(theta)+.5
            #print(xp)
            proj[i][0]=xp
    ...
    for i in range(len(proj)):
        for j in range(len(proj)):
            if i!=j:
                ...
                p2x=proj[j][0]
                p2y=proj[j][1]
                graph.create_line(scale*p1x-00,scale*p1y+off,scale*p2x-00,scale*p2y+off,width=5)
                #print(scale*p1x-00,scale*p1y+off,scale*p2x-00,scale*p2y+off)
