...
while cont==1:
    ...
    for i in range(len(info)):
        ...
        yp=(use[1]-obsy)/(2*dxz)*tan(theta)+.5#(ryz*cos(phix-betax)-obsy)/(2*dxz)*tan(theta)+.5
        #print(yp)
        proj[i][1]=yp
        ...
        #print("HELO")
        xp=(use[0]-obsx)/(2*dyz+dd)*tan(theta)+.5#(rxz*cos(phiz-betaz)-obsx)/(2*dyz+dd)*tan(theta)+.5
        #print(xp)
        proj[i][0]=xp
